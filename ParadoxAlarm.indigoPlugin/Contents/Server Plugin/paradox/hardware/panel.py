...
logger = logging.getLogger("Plugin.PAI").getChild(__name__)
...
class Panel:
    ...
    async def load_memory(self):
        ...
        #logger.error("SendMessage Labels Loaded")
    ...
    async def load_labels(self):
        ...
        for elem_type in self.mem_map["labels"]:
            ...
            if elem_type.title()=="Zone":
                logger.debug("Storing Zone Labels to self.zoneLabels")
                self.panel_labels = data #f"{', '.join([v['label'] for v in data[elem_type].values()])}"
            if elem_type.title()=="Partition":
                logger.debug("Storing Partition Labels to self.partition_labels")
                self.partition_labels = data
            logger.info(

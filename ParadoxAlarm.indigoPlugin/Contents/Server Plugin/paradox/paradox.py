...
logging.getLogger("Plugin.PAI").setLevel(logging.DEBUG)

class Paradox:
    def __init__(self, plugin, retries=3):
        #self.logger = logging.getLogger("Plugin.ParadoxAlarm")  # .getChild(__name__)
        #logger.debug("Message Starting Here from paradox Library")
        ...
        self.indigo_status = None
       # self.zoneLabels = None
        ...
        if not self._connection:
            ...
            if cfg.CONNECTION_TYPE == "Serial":
                ...
                if cfg.IP_CONNECTION_BARE:
                    ...
                    logger.info("Using Local IP Connection:"+str(cfg.IP_CONNECTION_HOST)+":"+str(cfg.IP_CONNECTION_PORT))
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if initiate_reply:
                ...
                self.plugin.panelID =  "Panel {} version {}".format(model, firmware_version)
            else:
    ...
    async def full_connect(self) -> bool:
        try:
            ...
            logger.info("Running.  Connected to Device and Panel.  All Systems Okay.")
    ...
    async def loop(self):
        ...
        while self.run_state not in (RunState.STOP, RunState.ERROR) and self.plugin.able_to_connect:
            tstart = time.time()
    ...
    def _process_status(raw_status: Container) -> None:
        ...
        if cfg.LOGGING_DUMP_STATUS:
            logger.debug("properties: {}".format(status))
    ...
    async def control_zone(self, zone: str, command: str) -> bool:
        ...
        logger.info("Zone Selected: "+str(zones_selected))
        # Apply state changes
        accepted = False
        try:
    ...
    async def control_partition(self, partition: str, command: str) -> bool:
        ...
        logger.info("Partition Selected:"+str(partitions_selected))
        # Not Found
    ...
    async def control_output(self, output, command) -> bool:
        ...
        try:
            ...
            logger.debug("Control Output Result: {}".format(accepted) )
        except NotImplementedError:
    ...
    async def send_panic(self, partition_id, panic_type, user_id) -> bool:
        ...
        #logger.debug("{}".format(partition))
        user = self.storage.get_container_object("user", user_id)
      #  user = self.storage.get_container("user", 1)
        logger.debug("user")
    ...
    #### Routines Added
    def que_panel_command(self):
        completed = self.work_loop.run_until_complete(self.control_partition("1", "DISARM"))
        logger.debug("Command Feedback:" + str(completed))
        #asyncio.get_event_loop().create_task(self.control_zone(zone, command))
    ...
    def _on_property_change(self, change: Change):
        ...
        try:
            ...
            ps.sendEvent(event)
            self.plugin.manage_event(event)
